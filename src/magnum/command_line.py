...
def parse(argv, version):
    ...
    hw_group = optparse.OptionGroup(parser,
        "Hardware options",
    ...
    log_group = optparse.OptionGroup(parser,
        "Logging options",
    ...
    def prange_callback(option, opt_str, value, parser, *args, **kwargs):
        ...
        if not p0 < p1:
            raise optparse.OptionValueError("Invalid -p argument specified: second value must be greater than first value")
        setattr(parser.values, "prange", range(p0, p1))

    ctrl_group = optparse.OptionGroup(parser,
    ...
    misc_group.add_option("--on-io-error",
        type = "int",
